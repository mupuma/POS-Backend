...
class Sage300InventoryService {
   ...
   */
  async getInventoryByPrefix(itemPrefix, locations = null) {
    try {
      const whereClause = {
        ITEMNO: {
          [Op.like]: `${itemPrefix}%`
      ...
      if (locations !== null) {
        ...
        whereClause.LOCATION = {
          [Op.in]: locations.map(loc => loc.trim().padEnd(6, ' '))
        };
      }
  ...
  async syncInventoryToDatabase(models, itemPrefix = 'CP', locationStoreMap = {}) {
    ...
    try {
      // Get locations from mapping to filter Sage 300 query
      const locations = Object.keys(locationStoreMap);

      // Fetch inventory from Sage 300 database
      const inventoryData = await this.getInventoryByPrefix(itemPrefix, locations);
   ...
   */
  async getInventorySummary(itemPrefix = 'CP', locations = null) {
    try {
      const whereClause = {
        ITEMNO: {
          [Op.like]: `${itemPrefix}%`
