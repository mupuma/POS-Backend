...
class ZRAIntegrationService {
    ...
    /**
     * Calculate VAT amounts for tax-inclusive amounts
     * @param {number} taxInclusiveAmount - Amount with VAT included
     * @param {number} taxRate - Tax rate percentage
     * @returns {object}
     */
    calculateVATFromInclusive(taxInclusiveAmount, taxRate = 16) {
        const taxableAmount = taxInclusiveAmount / (1 + (taxRate / 100));
        const taxAmount = taxInclusiveAmount - taxableAmount;
    ...
    async transformToZRASalesData(saleData, items, user) {
        ...
        const itemList = items.map((item, index) => {
            // Use the tax-inclusive total price for calculations
            const taxInclusiveAmount = parseFloat(item.tax_inclusive_total) || 
                (item.unit_price * item.quantity * (1 + ((item.tax_rate || 16) / 100)));
            
            const taxableAmount = parseFloat(taxInclusiveAmount / (1 + (16 / 100)));
            const taxAmount = taxInclusiveAmount - taxableAmount;
            ...
            return {
                ...
                tlCatCd: null,
                iplCatCd: null,
                vatTaxblAmt: parseFloat(taxableAmount.toFixed(4)),
                vatAmt: parseFloat(taxAmount.toFixed(4)),
                exciseTaxblAmt: 0,
                tlTaxblAmt: 0,
                iplTaxblAmt: 0,
                iplAmt: 0,
        ...
        return {
            ...
            totAmt: parseFloat(totalTaxableAmountA.toFixed(4)) + parseFloat(totalTaxAmountA.toFixed(4)),
            prchrAcptcYn: "N",
    ...
    transformToZRAStockItemsData(saleData, items, user) {
        ...
        return {
            ...
            totAmt: parseFloat(totalTaxableAmount.toFixed(4)) + parseFloat(totalTaxAmount.toFixed(4)),
            remark: null,
