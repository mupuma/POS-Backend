...
class ZRAIntegrationService {
    constructor() {
        ...
        timeout: parseInt(process.env.ZRA_TIMEOUT_MS) || 30000,
    ...
    async processZRAIntegration(saleData, items, user) {
        ...
        try {
            ...
            if (stockMasterData && stockMasterData.stockItemList) {
                for (const item of stockMasterData.stockItemList) {
                    const productId = items.find(i => (i.product?.product_code || `ITEM${i.product_id}`) === item.itemCd)?.product_id;
                    const storeId = user?.store_id;
                    ...
                    if (productId && storeId) {
                        try {
                            ...
                            console.error('Error checking min stock level in processZRAIntegration:', e.message);
                        }
                    }
            ...
            if (!belowMinimum) {
                ...
                console.log('Skipping ZRA stock master update because stock levels are below minimum');
                results.responses.push({
                    success: true,
                    skipped: true,
                    endpoint: 'saveStockMaster',
                    message: 'Skipped due to stock levels below minimum'
                });
            }
