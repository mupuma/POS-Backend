...
class SageOrdersService {
  constructor() {
    this.timeout = 300000; // 5 min timeout for large orders with many lines
  }
  ...
  _normalizeItem(item) {
    ...
    const order = {
      ...
      OrderType: 'Active',
      OrderDate: orderDate,
        OrderDescription: `${user?.store?.store_location} POS Sales - ${orderDate}`,
    ...
    try {
      const response = await axios.post(
        `${process.env.SAGE_BASE_URL || 'http://10.40.0.42/Sage300WebApi/v1.0/-/DAPTST'}/OE/OEOrders`,
        order,
        {
          headers: {
