...
const ZRAIntegrationService = require('./generateSmartInvoiceCreditNote');
...
class AccountsReceivableBatchReturn {
     ...
     */
    async createConsolidatedArBatchReturn(salesDataArray, user, date) {
    ...
    async createSageArBatchReturn(salesData, items, user) {
        const salesArray = [{
            items: items,
            salesData: salesData
        }];

        return this.createConsolidatedArBatchReturn(salesArray, user);
    }

}

module.exports = AccountsReceivableBatchReturn;