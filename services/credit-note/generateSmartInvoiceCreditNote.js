 ...
 */
class ZRACreditNoteIntegrationService {
    constructor() {
     ...
     */
    async generateCISCreditNoteNumber(store_id) {
        try {
            ...
            // Construct the new credit note number
            const newCreditNoteNumber = `${prefix}${newNumber}`;

            // Update the store with the new credit note number
            await storeObj.update({ credit_note_number: newCreditNoteNumber });

            return newCreditNoteNumber;

        } catch (error) {
            console.error('Error generating CIS credit note number:', error);
            throw error;
        }
    }
     ...
     */
    async transformToZRACreditNoteSalesData(saleData, items, user,reasonCode,originalInvoiceNo,sdcid) {
        const currentDateTime = this.formatZRADateTime();
        const currentDate = this.formatZRADate();

        // Generate credit note number with CRN prefix
        const cisCreditNoteNo = await this.generateCISCreditNoteNumber(user.store_id);
        ...
        return {
            ...
            destnCountryCd: "",
            dbtRsnCd: "",
            invcAdjustReason: saleData.notes,
            itemList
        };
    }
     ...
     * @returns {object}
     */
    transformToZRACreditNoteStockItemsData(saleData, items, user) {
        const currentDate = this.formatZRADate();
     ...
     * @returns {object}
     */
    transformToCreditNoteZRAStockMasterData(items, user, remainingQty) {
     ...
     * @returns {Promise}
     */
    async sendCreditNoteStockItemsData(stockItemsData) {
        try {
     ...
     * @returns {Promise}
     */
    async sendCreditNoteStockMasterData(stockMasterData) {
        try {
    ...
    async processZRAIntegration(saleData, items, user) {
        ...
        try {
            ...
            const salesData = await this.transformToZRACreditNoteSalesData(saleData, items, user);
            const stockItemsData = this.transformToZRACreditNoteStockItemsData(saleData, items, user);
            const stockMasterData = this.transformToCreditNoteZRAStockMasterData(items, user);

            // Send requests to all three endpoints sequentially to avoid race conditions
            const salesResponse = await this.sendCreditNoteSalesData(salesData);
            ...
            const stockItemsResponse = await this.sendCreditNoteStockItemsData(stockItemsData);
            ...
            const stockMasterResponse = await this.sendCreditNoteStockMasterData(stockMasterData);
...
module.exports = ZRACreditNoteIntegrationService;